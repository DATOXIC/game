...
def game():
    ...
    while running:
        ...
        # Display score
        score_font=pygame.font.Font("pixelfont.ttf",35)
...
def car_selection_menu():
    ...
    while selecting:
        ...
        title_fonts= pygame.font.Font("pixelfont.ttf",70)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_LEFT:  # Chuyển sang xe trước đó
                    selected_car_index = (selected_car_index - 1) % len(car_images)
                elif event.key == pygame.K_RIGHT:  # Chuyển sang xe tiếp theo
                    selected_car_index = (selected_car_index + 1) % len(car_images)
